...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    const galleryItems = sourceItems.concat(sourceItems);
    this.mediasImages = galleryItems;
    this.medias = this.mediasImages.map((data, index) => {
      const raw = sourceItems[index % sourceItems.length];
      return new Media({
  ...
  onTouchUp(e) {
    ...
    if (dist < 10 && this.medias && this.medias.length) {
      ...
      if (closestMedia) {
        ...
        if (this.onItemClick) {
          this.onItemClick(closestMedia.rawItem);
        }
      }
    }
  ...
  update() {
    if (this.autoSpin && !this.isDown && !this.isHovered) {
      this.scroll.target += this.autoSpinSpeed * 0.04;
    }
