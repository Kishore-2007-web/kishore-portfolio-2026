...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill, onClick, ariaLabel }) {
  ...
  return (
    <motion.div
      ...
      role="button"
      tabIndex={0}
      aria-label={ariaLabel || `Orbit item ${index + 1}`}
  ...
  const items = useMemo(() => {
    ...
    return images.map((src, index) => (
      <img
        ...
        src={typeof src === 'string' ? src : src?.image || src}
        alt={labels[index] || `${altPrefix} ${index + 1}`}
        draggable={false}
        className="orbit-image"
      />
    ));
  }, [images, customItems, labels, altPrefix]);
  ...
  return (
    <div
      ...
      style={{
        ...
        aspectRatio: responsive ? (shape === 'ellipse' ? '2.4 / 1' : '1 / 1') : undefined,
      }}
    >
      <div
